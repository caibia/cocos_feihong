...
class EditorWindowManager:
    ...
    # FairyGUI 编辑器窗口标题关键词（按优先级排序，匹配大小写不敏感）
    # 注意：FairyGUI 编辑器窗口标题里的工程名沿用磁盘上的文件夹名，
    # 本仓库工程目录是 "UIProject"（无下划线），因此必须把 "uiproject" 放在第一位匹配；
    # 旧的 "ui_project" 保留作为对带下划线命名的兼容兜底。
    WINDOW_PATTERNS = ["uiproject", "ui_project"]
