...
/**
 * Spine 拖拽管理器，负责骨骼预览对象的拖放代理。
 */
export class DragSpineManager {
     ...
     * 单例实例引用。
     */
    private static _inst: DragSpineManager;
    /**
     * 获取当前单例实例。
     */
    public static get inst(): DragSpineManager {
        if (!DragSpineManager._inst)
            DragSpineManager._inst = new DragSpineManager();
        return DragSpineManager._inst;
    }

    /**
     * 初始化 Spine 拖拽代理、根节点引用和拖拽状态。
     */
    public constructor() {
    ...
    public startDrag(url: string | null, sourceData?: any, touchId?: number): void {
        ...
        this._spineUnit.play(url, null, pt.x, pt.y, this._l3d);
